<?php
/**
 * Tour Gallery — Single Page Mosaic + Thumbnail Strip
 *
 * Path: themes/moga-travel/template-parts/tour/single-gallery.php
 *
 * Layout:
    ...
    <div class="moga-gallery-mosaic moga-gallery-mosaic--<?php echo esc_attr( $mosaic_count ); ?>" data-total="<?php echo esc_attr( $gallery_count ); ?>">
        ...
        <?php foreach ( $mosaic_ids as $index => $attachment_id ) :
            ...
            $img_alt  = get_post_meta( $attachment_id, '_wp_attachment_image_alt', true )
                ?: ( get_the_title( $tour_id ) . ' — ' . sprintf( __( 'photo %d', 'moga-travel' ), $index + 1 ) );
            if ( ! $img_url ) continue;
        ?>
            <a
                ...
                data-glightbox="gallery: <?php echo esc_attr( $gallery_key ); ?>; description: <?php echo esc_attr( get_the_title( $tour_id ) ); ?>"
                aria-label="<?php printf( esc_attr__( 'Photo %1$d of %2$d — %3$s', 'moga-travel' ), $index + 1, $gallery_count, get_the_title( $tour_id ) ); ?>"
            >
                <img
        ...
        <?php foreach ( $thumb_ids as $attachment_id ) :
            $img_full = wp_get_attachment_image_url( $attachment_id, 'full' );
            $img_alt  = get_post_meta( $attachment_id, '_wp_attachment_image_alt', true ) ?: get_the_title( $tour_id );
    ...
    <?php if ( ! empty( $strip_ids ) ) : ?>
        <div class="moga-gallery-thumbs" role="list" aria-label="<?php esc_attr_e( 'More photos', 'moga-travel' ); ?>">
            <?php foreach ( $strip_ids as $ti => $attachment_id ) :
                ...
                $alt       = get_post_meta( $attachment_id, '_wp_attachment_image_alt', true ) ?: get_the_title( $tour_id );
            ...
            ?>
                <a
                    ...
                    data-glightbox="gallery: <?php echo esc_attr( $gallery_key ); ?>"
                    role="listitem"
                    aria-label="<?php printf( esc_attr__( 'Photo %d — %s', 'moga-travel' ), $mosaic_count + $ti + 1, get_the_title( $tour_id ) ); ?>"
                >
                    <img
