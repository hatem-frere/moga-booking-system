 ...
 *
 * Displays a single property in grid layout.
 * Used on: homepage, search results (grid mode).
 *
 * Data passed via moga_get_part():
 *   $data['property_id'] — Post ID (optional, defaults to current post)
 *
 * @package    MogaTravel
 * @since      1.0.0
 */
...
$data        = function_exists( 'moga_part_data' ) ? moga_part_data() : array();
$property_id = isset( $data['property_id'] ) ? intval( $data['property_id'] ) : get_the_ID();
...
<article class="moga-card moga-card--grid moga-property-card" data-id="<?php echo esc_attr( $property_id ); ?>">
