...
// Owner contact. Property has NO per-listing name/photo override
// fields (unlike Tour) — only phone/whatsapp/email are overridable
// per listing; name and photo always come from the vendor's own
// profile. Confirmed directly against class-moga-cpt-property.php
// and render_property_contact() before building this.
$owner_phone    = get_post_meta($property_id, '_moga_phone', true);
$owner_whatsapp = get_post_meta($property_id, '_moga_whatsapp', true);
$owner_email    = get_post_meta($property_id, '_moga_email', true);

$owner_author_id = (int) get_post_field('post_author', $property_id);
$owner_author    = get_userdata($owner_author_id);
$owner_company   = get_user_meta($owner_author_id, 'moga_company_name', true);
$owner_name      = $owner_company ?: ($owner_author ? $owner_author->display_name : '');

$owner_photo_id  = (int) get_user_meta($owner_author_id, 'moga_profile_photo', true);
$owner_photo_url = $owner_photo_id ? wp_get_attachment_image_url($owner_photo_id, 'medium') : '';

if (! $owner_phone) {
    $owner_phone = get_user_meta($owner_author_id, 'moga_contact_phone', true);
}
if (! $owner_whatsapp) {
    $owner_whatsapp = get_user_meta($owner_author_id, 'moga_contact_whatsapp', true);
}
if (! $owner_email) {
    $owner_email = get_user_meta($owner_author_id, 'moga_contact_email', true);
}
                    ...
                    <?php if ($owner_name) : ?>
                        <div class="moga-organizer-card">
                            <?php if ($owner_photo_url) : ?>
                                <img src="<?php echo esc_url($owner_photo_url); ?>" alt="<?php echo esc_attr($owner_name); ?>" class="moga-organizer-card__photo" width="56" height="56">
                            <?php else : ?>
                                <div class="moga-organizer-card__photo moga-organizer-card__photo--placeholder">
                                    <?php echo esc_html(mb_substr($owner_name, 0, 1)); ?>
                                </div>
                            <?php endif; ?>
                            <div class="moga-organizer-card__info">
                                <span class="moga-organizer-card__label"><?php esc_html_e('Hosted by', 'moga-travel'); ?></span>
                                <span class="moga-organizer-card__name"><?php echo esc_html($owner_name); ?></span>
                            </div>
                        </div>
                    <?php endif; ?>

                    <?php // ---- Message the Owner ----
                    ?>
                    <?php
                    get_template_part('template-parts/global/vendor-contact-form', null, array(
                        'vendor_name'     => $owner_name,
                        'vendor_email'    => $owner_email,
                        'vendor_phone'    => $owner_phone,
                        'vendor_whatsapp' => $owner_whatsapp,
                        'listing_title'   => $title,
