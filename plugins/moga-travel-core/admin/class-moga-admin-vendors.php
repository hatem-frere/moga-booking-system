...
class Moga_Admin_Vendors
{
    ...
    public static function render_status_column($output, $column_name, $user_id)
    {
        ...
        $status = get_user_meta($user_id, 'moga_vendor_status', true) ?: 'pending';
    ...
    public static function add_row_actions($actions, $user)
    {
        ...
        $status = get_user_meta($user->ID, 'moga_vendor_status', true) ?: 'pending';
        ...
        <div class="wrap">
            ...
            <?php if (isset($_GET['moga_notice'])) : ?>
                <div class="notice notice-success is-dismissible">
                    <p><?php esc_html_e('Vendor updated.', 'moga-travel-core'); ?></p>
                </div>
            <?php endif; ?>
            ...
            <table class="wp-list-table widefat fixed striped" style="margin-top:16px;">
                ...
                <tbody>
                    <?php if (empty($vendors)) : ?>
                        <tr>
                            <td colspan="7"><?php esc_html_e('No vendor accounts yet.', 'moga-travel-core'); ?></td>
                        </tr>
                    <?php endif; ?>

                    <?php foreach ($vendors as $vendor) :
                        // BUG FIX (Aug 19 session): was '?: approved',
                        // with a comment claiming this treats legacy
                        // accounts as approved for backward
                        // compatibility. That default directly
                        // conflicted with Moga_Roles::is_vendor_approved(),
                        // which has no such exception and already
                        // treats an unset status as NOT approved
                        // everywhere it actually matters (whether a
                        // listing can publish) — so an unreviewed
                        // vendor showed green "Approved" here while
                        // being correctly held back behind the scenes.
                        // Now consistent: unset displays as 'pending',
                        // matching the real gate.
                        $status       = get_user_meta($vendor->ID, 'moga_vendor_status', true) ?: 'pending';
...
<?php
    ...
    public static function handle_action()
    {
        ...
        if (
            ! $user_id || ! isset($_REQUEST['moga_vendor_nonce'])
